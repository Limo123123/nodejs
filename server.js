// server.js - Finale Version mit Login, Usern, Guthaben, Inventar-Basis etc.
...
const localPathToSecretEnv = path.resolve(__dirname, 'secret.env'); // Lokaler Pfad (optional)

let configPath;
if (fs.existsSync(pathToSecretEnv)) {
    configPath = pathToSecretEnv;
    console.log(`Lade .env aus Render Secret File: ${configPath}`);
} else if (fs.existsSync(localPathToSecretEnv)) {
    configPath = localPathToSecretEnv;
    console.log(`Lade .env aus lokaler Datei: ${configPath}`);
} else {
     console.warn(`Keine Secret File unter ${pathToSecretEnv} oder ${localPathToSecretEnv} gefunden. Nutze nur System Env Vars oder Defaults.`);
}
...
const app = express();
app.set('trust proxy', 1); // WICHTIG für secure cookies hinter Proxy (wie bei Render)

const HTTP_PORT = process.env.PORT || 80;
const SEED_PRODUCTS_FILE = 'products_seed.json'; // Name deiner Seed-Datei
...
// Baue URI nur zusammen, wenn User/Passwort da sind UND keine komplette URI in .env steht
const mongoClusterAddress = process.env.MONGO_CLUSTER_ADDRESS || "limodb.kbacr5r.mongodb.net"; // Default, falls nicht in .env
const mongoAppName = process.env.MONGO_APP_NAME || "LimoDB"; // Default, falls nicht in .env

const mongoUri = mongoUriFromEnv || 
                 (mongoUser && mongoPassword ? 
                    `mongodb+srv://${mongoUser}:${mongoPassword}@${mongoClusterAddress}/?retryWrites=true&w=majority&appName=${mongoAppName}` 
                    : null);
...
const inventoriesCollectionName = 'userInventories'; // NEU für Inventar
...
const frontendDevUrlHttp = 'http://127.0.0.1:8080'; // Dein lokaler Live Server HTTP Port
const frontendDevUrlHttps = 'https://127.0.0.1:8080'; // Falls du lokal HTTPS nutzt
...
// --- Hilfsfunktionen ---
async function generateUniqueId(collection = productsCollection) { // Standardmäßig für Produkte
    let newId; let idExists = true; let attempts = 0; const maxAttempts = 1000;
    while (idExists && attempts < maxAttempts) {
        newId = Math.floor(100000 + Math.random() * 900000);
        try { const existing = await collection.findOne({ id: newId }, { projection: { _id: 1 } }); if (!existing) idExists = false; }
        catch (findErr) { console.error(`❌ Fehler ID-Prüfung ${newId}:`, findErr); throw new Error('DB-Fehler ID-Gen.'); } attempts++;
    } if (idExists) throw new Error('Fehler ID-Gen., Kollisionen.'); console.log(`   🔑 Neue ID für ${collection.collectionName}: ${newId}`); return newId;
}
    ...
    try {
        ...
        for (const prod of parsedData.products) {
            ...
            try {
                ...
                productsToSeed.push({ id: newId, name: prod.name.trim(), price: prod.price && typeof prod.price === 'string' ? prod.price.trim() : "$0.00", image_url: prod.image_url && typeof prod.image_url === 'string' ? prod.image_url.trim() : `https://via.placeholder.com/150x160.png?text=${encodeURIComponent(prod.name)}`, stock: 20, default_stock: 20 });
        ...
        if (productsToSeed.length > 0) {
            ...
            try { const ir = await productsCollection.insertMany(productsToSeed, { ordered: false }); seededCount = ir.insertedCount; console.log(`   ✅ DB mit ${seededCount} Produkten befüllt.`); }
            catch (imErr) { console.error('❌ Fehler insertMany Seed:', imErr); seededCount = imErr.result ? imErr.result.nInserted : 0; console.error(`   Nur ${seededCount} eingefügt.`); }
...
async function resetProductStock() { 
  ...
  try { const r = await productsCollection.updateMany( { id: { $type: 'number', $gte: 100000 } }, [ { $set: { stock: { $ifNull: ["$default_stock", 20] } } } ]); console.log(`♻️ Bestand für ${r.modifiedCount} Produkte auf Default.`); }
  catch (error) { console.error('❌ Fehler beim Reset auf Default:', error); throw error; }
}

async function zeroOutStock() { 
  console.warn('!!! ACHTUNG: Setze Stock ALLER Produkte auf 0 !!!');
  try { const r = await productsCollection.updateMany( { id: { $type: 'number', $gte: 100000 } }, { $set: { stock: 0 } } ); console.log(`♻️ Bestand für ${r.modifiedCount} Produkte auf 0.`); }
  catch (error) { console.error('❌ Fehler Nullsetzen:', error); throw error; }
}
...
MongoClient.connect(mongoUri)
  .then(async client => {
    ...
    inventoriesCollection = db.collection(inventoriesCollectionName); // Inventar Collection
    ...
    try { 
        ...
        console.log('✅ Indizes erstellt/existieren.'); 
    }
...
app.delete('/api/products/:id', isAdmin, async (req, res) => { const id = parseInt(req.params.id, 10); if (isNaN(id) || id<100000) return res.status(400).json({ error: 'Ungültige ID!' }); try { await inventoriesCollection.deleteMany({productId: id}); /* Erst Inventare löschen */ const result = await productsCollection.deleteOne({ id: id }); if (result.deletedCount === 0) return res.status(404).json({ error: 'Produkt nicht gefunden!' }); res.json({ message: `Produkt ${id} und zugehörige Inventareinträge gelöscht!` }); } catch (err) { res.status(500).json({ error: 'Fehler beim Löschen!' }); } });
...
app.post('/api/purchase', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        if (validUpdates.length > 0) {
            ...
            console.log(`   -> Bestand für ${productUpdateResult.modifiedCount} Produkte reduziert.`); 
            if (inventoryOps.length > 0) { await inventoriesCollection.bulkWrite(inventoryOps); console.log("   -> Inventar aktualisiert.");}
