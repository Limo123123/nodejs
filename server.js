...
let limChatsCollection, limMessagesCollection;
let ideasCollection;
...
app.get('/api/chat/chats', isAuthenticated, async (req, res) => {
    try {
        ...
        // Lädt NUR NOCH Tinda Chats, ignoriert alles andere
        const userChats = await limChatsCollection.find({ 
            participants: userId,
            type: { $in: ['tinda', 'tinda_child', 'tinda_family'] } 
        })
        .sort({ updatedAt: -1 })
        .limit(100)
        .toArray();
        ...
        res.status(500).json({ error: "Fehler beim Laden der Tinda Chats." });
    }
});
    ...
    try {
        ...
        if (!chat) {
            ...
            const newChat = {
                ...
                classifiedAdId: ad._id,
                adTitle: ad.title,
                sellerId: ad.sellerId, // <--- DIESE ZEILE HAT BEI DIR GEFEHLT!
...
// 10. Ein Angebot in den Chat senden (Nur Verkäufer)
...
// 11. Angebot annehmen oder ablehnen (Nur Käufer)
...
// 1. Raum hosten
app.post('/api/amongus/host', isAuthenticated, async (req, res) => {
...
// 2. Raum beitreten
app.post('/api/amongus/join', isAuthenticated, async (req, res) => {
...
app.get('/api/amongus/signal/:roomCode', isAuthenticated, async (req, res) => {
